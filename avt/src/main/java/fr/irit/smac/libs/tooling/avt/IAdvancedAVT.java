 ...
 * <p>
 * Pay caution, some of those methods don't guarantee the AVT state consistency.
 * Generally, you should only use this interface if you have good reasons and
 ...
 * this consistency are tagged with a <strong>STATE CONSISTENCY WARNING</strong>
 * </p>
 * 
 * @author Sylvain Lemouzy
 */
public interface IAdvancedAVT extends IAVT {
     ...
     * <p>
     * If the current value is lower than the new <code>lowerBound</code> then
     * the current value is set to <code>lowerBound</code>. In that case the AVT
     * state is reinitialized in order to keep state consistency.
     * </p>
     * <p>
     * The state consistency of the AVT is also ensured by the reconfiguration
     * of the DeltaManager from the new bounds values.
     * </p>
     * 
     * @param lowerBound
     ...
     * <p>
     * If the current value is higher than the new <code>upperBound</code> then
     * the current value is set to <code>upperBound</code>. In that case the AVT
     * state is reinitialized in order to keep state consistency.
     * </p>
     * <p>
     * The state consistency of the AVT is also ensured by the reconfiguration
     ...
     * <p>
     * Changing the criticity will change the AVT value accuracy estimation.
     * Thus, it will change the way the AVT adjusts its value :
     * </p>
     * <ul>
     * <li>If <code>criticity = 1</code> then the adjustment step will be big
     * </li>
     * <li>If <code>criticity = 0</code> then the adjustment step will be small
     * </li>
     * </ul>
     * 
     * @param value
