...
public class AgentLoggingExample {
    ...
    public static void main(String[] args) {
        ...
        AgentLog.initializer() // all the above lines are optional except the
                               // last one : .initialize();
            .logFolderName("log") // the name of the folder containing log files
                                  // (default "log")
            .clearLogFolderOnStartup(true) // if true, all the content of the
                                           // log folder (default true)
                                           // will be cleared at each
                                           // initialization
            // .customLogbackConfigurationFile("./custom-conf-logback.xml") //
            // if not set, the default configuration is set (default null)
            // if not null then override all the following configuration lines
            .logEnabled(true) // enable or disable logging (if disabled the log
                              // folder will not be cleared even if
            ...
            // .logPatternLayout("%replace(%5property{amasStepNumber}){\"\\s\",\"0\"}
            // > [%-6.6marker] %-7([%level]) %msg%n") // sets the output format
            // of log lines (default value : null)
            // // if set to a non null value then override the
            // // behind configuration lines
            // // for more informations see
            // http://logback.qos.ch/manual/layouts.html#ClassicPatternLayout
            // .logItemSeparator("...") // the separator between folowing
            // elements
            // .stepNumberDisplay()
            // .minSize(6)
            // .align(Side.RIGHT) // justification side if the string length is
            // lower than minSize
            // .displayed(true) // whether it is displayed or not
            // .prefix("=>") // sets the prefix to be displayed before the log
            // element (by default prefix and suffix are empty strings)
            // .order(2) // the order priority in the line (lower values are
            // diplayed first)
            // .parent() // used to comme back to AgentLog.initializer()
            // instance and configure other things
            //
            // .logLevelDisplay()
            // .align(Side.LEFT)
            // .displayed(true)
            ...
            // .prefix("(") // sets the prefix to be displayed before the log
            // element
            // .suffix(")") // sets the suffix to be displayed after the log
            // element
            // .parent()
            //
            ...
            // .minSize(12)
            // .maxSize(12)
            // .truncatedSide(Side.LEFT) // truncation side when the string
            // length to display is greater than maxSize
            // .prefix("{")
            // .suffix("}")
            // .order(3)
            // .parent()
